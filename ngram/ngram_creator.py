...
class NGramCreator:
        ...
        if kind == "ip_list":
            with open(self.training_file) as input_file:
                for line in tqdm(input_file, desc=self.training_file, total=self.training_file_lines,
                                 disable=self.disable_progress, miniters=1000, unit="pw"):
                    line = line.rstrip('\r\n')
                    if len(
                            line) != self.length:  # Important to prevent generating "passwor", or "iloveyo", or "babygir"
                        continue
                    if self._is_in_alphabet(line):  # Filter non-printable
                        ...
                        self.ip_list[self._n2iIP(ngram)] = self.ip_list[
                                                               self._n2iIP(ngram)] + 1  # Increase IP ngram count by 1
        elif kind == "cp_list":
            with open(self.training_file) as input_file:  # Open trainingfile
                for line in tqdm(input_file, desc=self.training_file, total=self.training_file_lines,
                    ...
                    if self._is_in_alphabet(line):  # Filter non-printable
                        old_pos = 0
                        for new_pos in range(self.ngram_size, len(line) + 1,
                            ...
                            self.cp_list[self._n2iCP(ngram)] = self.cp_list[self._n2iCP(
                                ngram)] + 1  # Increase CP ngram count by 1
        elif kind == "ep_list":
            with open(self.training_file) as input_file:  # Open trainingfile
                for line in tqdm(input_file, desc=self.training_file, total=self.training_file_lines,
                                 disable=self.disable_progress, miniters=1000, unit="pw"):
                    ...
                    if self._is_in_alphabet(line):  # Filter non-printable
                        ...
                        self.ep_list[self._n2iIP(ngram)] = self.ep_list[
                                                               self._n2iIP(ngram)] + 1  # Increase EP ngram count by 1
        else:
    ...
    def save(self, kind):
        ...
        with open('trained/' + file[:-4] + '_' + kind + '_' + str(self.ngram_size) + '_' + str(self.length) + '.pack',
                  'wb') as fp:
            if kind == "ip_list":
